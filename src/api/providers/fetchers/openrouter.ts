...
export async function getOpenRouterModels(options?: ApiHandlerOptions): Promise<Record<string, ModelInfo>> {
	...
	try {
		const response = await axios.get<OpenRouterModelsResponse>(`${baseURL}/models`, { timeout: MODEL_FETCH_TIMEOUT_MS })
...
export async function getOpenRouterModelEndpoints(
	...
	try {
		const response = await axios.get<OpenRouterModelEndpointsResponse>(`${baseURL}/models/${modelId}/endpoints`, { timeout: MODEL_FETCH_TIMEOUT_MS })
